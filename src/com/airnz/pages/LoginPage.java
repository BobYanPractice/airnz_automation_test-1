...
/**
 * LoginPage - Page Object for Air New Zealand Login Page.
 * Handles user authentication including login, registration, and password recovery.
 * 
 * @author AirNZ Automation Team
 * @version 1.0
 */
public class LoginPage extends BasePage {
    ...
    /**
     * Switches to Airpoints login tab
     * @return LoginPage instance for chaining
     */
    public LoginPage switchToAirpointsLogin() {
        click(airpointsTab);
        logger.info("Switched to Airpoints login");
        return this;
    }
    
    /**
     * Performs Airpoints login
     * @param airpointsNumber Airpoints number
     * @param pin PIN
     * @return HomePage after successful login
     */
    public HomePage loginWithAirpoints(String airpointsNumber, String pin) {
        switchToAirpointsLogin();
        waitForVisible(By.cssSelector("[data-testid='airpoints-number'], input[name='airpointsNumber']"));
        type(airpointsNumberInput, airpointsNumber);
        type(airpointsPinInput, pin);
        click(loginButton);
        waitForPageLoad();
        logger.info("Logged in with Airpoints: {}", airpointsNumber);
        return new HomePage(driver);
    }
